        ...
        if (err) {
            ...
            for (let i = 0; i < allWishes.length; i++) {
                if (allWishes[i]._id == req.params._id) {
                    res.render('wish.hbs', { link: allWishes[i]._id, title: `${allWishes[i].name} sent you some wishes check out now`, name: allWishes[i].name, wish: allWishes[i].greet, src: allWishes[i].imgurl, msg: allWishes[i].message });
                }
            }
        }
    });
